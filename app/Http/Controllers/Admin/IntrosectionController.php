...
class IntrosectionController extends Controller
{
    ...
    public function update(Request $request, $langid)
    {
        ...
        $rules = [
            ...
            'intro_section_button_text' => 'nullable',
            'intro_section_button_url' => 'nullable',
        ...
        if ($request->filled('image')) {
            @unlink('assets/frontend/images/' . $bs->intro_bg);
        ...
        if ($request->filled('image_2')) {
            @unlink('assets/frontend/images/' . $bs->intro_bg2);
            $filename = uniqid() .'.'. $extImage2;
            @copy($image2, 'assets/frontend/images/' . $filename);

            $bs->intro_bg2 = $filename;
        }
        $bs->save();
