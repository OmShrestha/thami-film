        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-4">
                            <div class="card-title d-inline-block">Topics</div>
                        </div>
                        <div class="col-lg-3">
                            @if (!empty($langs))
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#createModal"><i
                                    class="fas fa-plus"></i> Add Topic</a>
                            <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete"
                                    data-href="{{route('admin.topic.bulk.delete')}}"><i class="flaticon-interface-5"></i> Delete
                            </button>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-12">
                            @if (count($topics) == 0)
                                <h3 class="text-center">NO TOPIC FOUND</h3>
                            @else
                                <div class="table-responsive">
                                    <table class="table table-striped mt-3">
                                        ...
                                        @foreach ($topics as $key => $topic)
                                            <tr>
                                                <td>
                                                    <input type="checkbox" class="bulk-check" data-val="{{$topic->id}}">
                                                </td>
                                                <td>{{convertUtf8($topic->name)}}</td>
                                                <td>
                                                    @if ($topic->status == 1)
                                                ...
                                                <td>{{$topic->serial_number}}</td>
                                                <td>
                                                    <a class="btn btn-secondary btn-sm editbtn" href="#editModal"
                                                       data-toggle="modal" data-topic_id="{{$topic->id}}"
                                                       data-name="{{$topic->name}}" data-status="{{$topic->status}}"
                                                       data-serial_number="{{$topic->serial_number}}">
                                                    ...
                                                    <form class="deleteform d-inline-block"
                                                          action="{{route('admin.topic.delete')}}" method="post">
                                                        @csrf
                ...
                <div class="card-footer">
                    <div class="row">
                        <div class="d-inline-block mx-auto">
                            {{$topics->appends(['language' => request()->input('language')])->links()}}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxForm" class="modal-form create" action="{{route('admin.topic.store')}}" method="POST">
                        @csrf
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxEditForm" class="" action="{{route('admin.topic.update')}}" method="POST">
                        @csrf
                        <input id="intopic_id" type="hidden" name="topic_id" value="">
                        <div class="form-group">
                            <label for="">Website **</label>
                            <select id="site" name="site_id" class="form-control" readonly="readonly" style="pointer-events: none;">
                                <option value="" selected disabled>Select Website</option>
