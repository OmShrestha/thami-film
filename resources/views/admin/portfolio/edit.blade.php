          ...
          <form id="ajaxForm" class="" action="{{route('admin.portfolio.update')}}" method="post">
            ...
            <div class="form-group">
              ...
              <div class="thumb-preview" id="thumbPreview1">
                <img src="{{asset('assets/frontend/images/portfolios/featured/'.$portfolio->featured_image)}}"
                  alt="User Image">
              </div>
              <br>
              <br>
  ...
  $(document).ready(function(){
    $.get("{{route('admin.portfolio.images', $portfolio->id)}}", function(data){
        for (var i = 0; i < data.length; i++) {
          $("#imgtable").append('<tr class="trdb" id="trdb'+data[i].id+'"><td><div class="thumbnail"><img style="width:150px;" src="{{asset('assets/frontend/images/portfolios/sliders/')}}/'+data[i].image+'" alt="Ad Image"></div></td><td><button type="button" class="btn btn-danger pull-right rmvbtndb" onclick="rmvdbimg('+data[i].id+')"><i class="fa fa-times"></i></button></td></tr>');
        }
    });
  });
