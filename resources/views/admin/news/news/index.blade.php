                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#createModal"><i
                                    class="fas fa-plus"></i> Add News</a>
                            <button href="{{ route('admin.wordpress.news.import') }}"
                                    class="btn btn-primary float-right btn-sm mr-2 wordpress-import-news"><i
                                    class="fas fa-plus"></i> Import news from Wordpress
                            </button>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped mt-3" id="basic-datatables">
                                        ...
                                        @foreach ($news as $key => $new)
                                            <tr>
                                                <td>
                                                    <input type="checkbox" class="bulk-check" data-val="{{$new->id}}">
                                                </td>
                                                <td><img src="{{asset('assets/frontend/images/news/'.$new->main_image)}}" alt=""
                                                         width="80"></td>
                                                <td>{{convertUtf8($new->category->name)}}</td>
                                                <td>{{convertUtf8(strlen($new->title)) > 30 ? convertUtf8(substr($new->title, 0, 30)) . '...' : convertUtf8($new->title)}}</td>
                                                <td>
                                                    @php
                                                ...
                                                <td>{{$new->serial_number}}</td>
                                                <td>
                                                    <form id="statusForm{{$new->id}}" class="d-inline-block"
                                                ...
                                                @if ($new->sidebar == 1)
                                                bg-success
                                                @elseif ($new->sidebar == 0)
                                                bg-danger
                                                @endif
                                                " name="sidebar"
                                                                onchange="document.getElementById('statusForm{{$new->id}}').submit();">
                                                            <option value="1" {{$new->sidebar == 1 ? 'selected' : ''}}>Enabled
                                                            </option>
                                                            <option value="0" {{$new->sidebar == 0 ? 'selected' : ''}}>Disabled
                                                            </option>
                                                        </select>
                                                    </form>
                                                </td>
                                                <td>
                                                    <a class="btn btn-secondary btn-sm"
                                                       href="{{route('admin.blog.edit', $new->id) . '?language=' . request()->input('language')}}">
                    ...
                    >
                        ...
                        <div class="form-group">
                            ...
                            <p
                                ...
                                class="mb-0 text-danger em"
                            ></p>
                            <p class="text-warning mb-0"><small>The higher the serial number is, the later the blog will be
                                    shown.</small></p>
                        </div>
                        <div class="form-group">
    ...
    <script>
        $(document).ready(function () {
            $("select[name='language_id']").on('change', function () {
                ...
                $("#news_category").removeAttr('disabled');
        ...
        $('.wordpress-import-news').on('click', function (e) {
            e.preventDefault();
