                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                            ...
                            <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete"
                                    data-href="{{route('admin.tag.bulk.delete')}}"><i class="flaticon-interface-5"></i> Delete
                            </button>
                        </div>
                    </div>
                </div>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a class="btn btn-secondary btn-sm editbtn" href="#editModal"
                                                       data-toggle="modal" data-tag_id="{{$tag->id}}" data-site_id="{{$tag->site_id}}"
                                                       data-name="{{$tag->name}}" data-status="{{$tag->status}}"
                                                       data-serial="{{$tag->serial}}">
                                                    ...
                                                    <form class="deleteform d-inline-block"
                                                          action="{{route('admin.tag.delete')}}" method="post">
                                                        @csrf
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxForm" class="modal-form create" action="{{route('admin.tag.store')}}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select id="site" name="site_id" class="form-control" readonly="readonly" style="pointer-events: none;">
                                <option value="" selected disabled>Select Website</option>
                                @foreach ($sites as $site)
                                    <option value="{{$site->id}}" @if(session('siteId') == $site->id) selected @endif>{{ ucfirst($site->name) }}</option>
                                @endforeach
                            </select>
                            <p id="errsite_id" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control ltr" name="topic_id">
                                <option value="" selected disabled>Select topic</option>
                                @foreach ($topics as $topic)
                                    <option value="{{$topic->id}}">{{$topic->name}}</option>
                                @endforeach
                            </select>
                            <p id="errtopic_id" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control ltr" name="serial" value=""
                                   placeholder="Enter Serial Number">
                            <p id="errserial" class="mb-0 text-danger em"></p>
                            <p class="text-warning"><small>The higher the serial number is, the later the blog category will be
                                    shown.</small></p>
                        </div>
                    </form>
                </div>
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxEditForm" class="" action="{{route('admin.tag.update')}}" method="POST">
                        @csrf
                        <input id="intag_id" type="hidden" name="tag_id" value="">
                        <div class="form-group">
                            <label for="">Website **</label>
                            <select id="site" name="site_id" class="form-control" readonly="readonly" style="pointer-events: none;">
                                <option value="" selected disabled>Select Website</option>
                        ...
                        <div class="form-group">
                            <label for="">Topic **</label>
                            <select id="intopic_id" name="topic_id" class="form-control">
                                <option disabled>Select topic</option>
                                @foreach ($topics as $topic)
                                    <option value="{{$topic->id}}">{{$topic->name}}</option>
                                @endforeach
                            </select>
                            <p id="eerrtopic_id" class="mb-0 text-danger em"></p>
                        </div>
                        <div class="form-group">
                            <label for="">Status **</label>
                        ...
                        <div class="form-group">
                            <label for="">Serial Number</label>
                            <input id="inserial" type="number" class="form-control ltr" name="serial" value=""
                                   placeholder="Enter Serial Number">
                            <p id="eerrserial" class="mb-0 text-danger em"></p>
                            <p class="text-warning"><small>The higher the serial number is, the later the blog category will be
                                    shown.</small></p>
                        </div>
                    </form>
