        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-5">
                            <div class="card-title d-inline-block">Types</div>
                        </div>
                        ...
                        <div class="col-lg-3 offset-lg-1 mt-2 mt-lg-0">
                            <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#createModal"><i
                                    class="fas fa-plus"></i> Add Type</a>
                            <button class="btn btn-danger float-right btn-sm mr-2 d-none bulk-delete"
                                    data-href="{{route('admin.vehicle-types.bulk.delete')}}"><i class="flaticon-interface-5"></i> Delete
                            </button>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped mt-3">
                                        ...
                                        @foreach ($types as $key => $type)
                                            <tr>
                                                <td><input type="checkbox" class="bulk-check" data-val="{{$type->id}}"></td>
                                                <td>{{convertUtf8($type->name)}}</td>
                                                <td>
                                                    <img src="{{ loadImageFor($type->image) }}"
                                                         alt="{{$type->name}}" width="80">
                                                </td>
                                                <td>
                                                    <a class="btn btn-secondary btn-sm editbtn" href="#editModal" data-toggle="modal"
                                                       data-name="{{$type->name}}" data-type_id="{{$type->id}}" data-image="{{$type->image}}">
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxForm" class="modal-form create" action="{{route('admin.vehicle-types.store')}}" method="POST">
                        @csrf
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="ajaxEditForm" class="" action="{{route('admin.vehicle-types.update', 1)}}" method="POST">
                        @csrf
                        @method('PUT')
                        <input id="intype_id" type="hidden" name="type_id" value="">
                        <div class="form-group">
                            <label for="">Logo ** </label>
                            <br>
